...
Add this file to your $HOME/.emacs.d/ directory
And create a $HOME/.emacs file with contents: 

(package-initialize)
(org-babel-load-file "~/.emacs.d/emacs.org")

Great emacs course [[https://cestlaz.github.io/stories/emacs/]] 

Great tuto and dotfiles  http://www.coli.uni-saarland.de/~slemaguer/emacs/main.html

Zzamboni init.el https://zzamboni.org/post/my-emacs-configuration-with-commentary/
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list 'load-path "~/.emacs.d/elpa/use-package-20190529.2040")
  (require 'use-package)
...
M-x package-install yasnippet-classic-snippets
    then you can access YASnippet <-> insert at point
...
** TODO Auto-complete - global
 ...
 #+BEGIN_SRC emacs-lisp 
   ...
   ;; 			  (if (not (minibufferp (current-buffer)))
      ...
      ;;     '((candidates . (list "vagnerrener@gmail.com"
      ;; 			 "leacna@gmail.com"
      ;; 			 "foo3@example.com"))
      ;;       (prefix . "^To: \\(.*\\)")))
...
** DONE Company-mode auto-complete and company-shell
...
Ctrl-u Ctrl-c . for getting date and time - you can configure it on the minibuffer as you like and you can edit other dates with it
...
C-x C-m auto-complete-mode
C-x C-m count-words
C-x C-m eval-buffer -> evaluates the buffer, so that you do not need to restart emacs
C-x C-m global-auto-complete-mode
C-x C-m linum-mode
C-x C-m linum-relative-mode
C-x C-m linum-relative-toggle
C-x C-m list-packages C-s packageName i x
C-x C-m list-packages - then install with "i-x"
C-x C-m load-theme <RET> name-of-theme
C-x C-m package-install RET package-name
C-x C-m package-refresh-content
C-x C-m package-safe-delete RET package-name
C-x C-m RET slime 
C-x C-m RET speedbar
C-x m	compose-mail
C-x 5 m	compose-mail-other-frame
C-x 4 m	compose-mail-other window
C-x C-m electric-pair-mode	regulates the ugly parens in emacs
C-x C-m set-frame-font RET (font name, font size, etc...) 
...
Or "C-x C-m customize-variable RET default-input-method
...
* TODO Custom themes 
   ...
   dark-laptop goldenrod graham granger gray30 green-phosphor  
   ...
   Also install "doom-themes" "color-theme-modern"
...
* DONE Grasp - org-capture 
  ...
  python $HOME/org~/grasp/server/grasp_server.py --path $HOME/org~/capture.org 2>/dev/null 
  and use the grasp addon icon on firefox/chromium to grasp contents 
  the file for capturing is "$HOME/org~/capture.org" 
...
;;		 (mu4e-send-delay-setup)
	...
	;;	 (add-hook 'mu4e-main-mode-hook 'mu4e-send-delay-initialize-send-queue-timer) 
      ...
      (setq mu4e-org-contacts-file "~/.mutt/mutt_emailAddrss")
...
* Scheme 
 ...
 2. M-x package-install RET geiser
 3. M-x package-install RET scheme-complete
...
* Trident convert lisp 2 javascript

 https://github.com/johnmastro/trident-mode.el

 Install packager dash, slime and trident  
